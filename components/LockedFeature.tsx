...
export default function LockedFeature({ feature, requiredPlan, children }: LockedFeatureProps) {
  ...
  return (
    <div className="rounded-2xl border border-orange-500/20 bg-orange-500/10 backdrop-blur p-5 flex flex-col items-center text-center gap-2">
      <div className="text-3xl select-none">🔒</div>
      <div className="font-bold text-white text-sm">{feature}</div>
      <p className="text-xs text-slate-400 leading-relaxed max-w-xs">
        此功能需要升级到{planLabel}才能解锁，帮助你更深入了解孩子的成长轨迹。
      </p>
      <div className="mt-1 text-xs font-semibold text-orange-300 bg-white/10 border border-orange-500/30 rounded-full px-4 py-1.5 cursor-pointer hover:bg-orange-500/10 transition-colors">
        升级 {planLabel} 解锁
      </div>
