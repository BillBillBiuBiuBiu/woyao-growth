"use client";
import { mockReports, mockRadarData } from "@/lib/mock-data";
...
function MetricsCards({ metrics }: { metrics: NonNullable<Report["metrics"]> }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-2">
      {items.map((item) => (
        <div key={item.label} className="bg-orange-500/10 rounded-2xl p-3 text-center">
          <div className="text-lg font-black text-orange-300">{item.value}</div>
...
function BasicReportLayout({ report, coachName }: { report: Report; coachName: string }) {
  return (
    <div className="flex flex-col gap-4 px-4">
      {/* Hero summary */}
      <div className="rounded-3xl bg-white/10 backdrop-blur border border-orange-500/20 shadow-sm p-4 relative overflow-hidden">
        <div className="flex items-center gap-1.5 mb-3">
          <span className="text-amber-400">⭐</span>
          <span className="text-sm font-bold text-amber-300">成长记录</span>
          <span className="text-amber-400">⭐</span>
        </div>
        <div className="relative px-2">
          <span className="absolute -top-1 -left-0 text-4xl font-black text-orange-200 leading-none select-none">"</span>
          <p className="text-base font-bold text-white leading-relaxed pt-3 pb-1 px-4">{report.summary}</p>
          <span className="absolute -bottom-2 right-0 text-4xl font-black text-orange-200 leading-none select-none">"</span>
        </div>
      </div>

      {/* Strengths */}
      <div className="rounded-3xl bg-white/10 backdrop-blur border border-green-500/20 shadow-sm p-4">
        <div className="flex items-center gap-1.5 mb-3">
          <span className="text-base">✅</span>
          <span className="text-sm font-bold text-white">本次进步点</span>
        </div>
      ...
      {report.weaknesses.length > 0 && (
        <div className="rounded-3xl bg-white/10 backdrop-blur border border-orange-500/20 shadow-sm p-4">
          <div className="flex items-center gap-1.5 mb-3">
            <span className="text-base">📌</span>
            <span className="text-sm font-bold text-white">下阶段练习重点</span>
          </div>
          <ul className="flex flex-col gap-2">
      ...
      {report.coachComment && (
        <div className="rounded-3xl bg-white/10 backdrop-blur border border-orange-500/20 shadow-sm p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              <div className="text-xs text-slate-400">教练寄语 ♡</div>
              <div className="text-sm font-bold text-white">{coachName || "教练"}</div>
            </div>
          </div>
          <p className="text-sm text-gray-200 leading-relaxed">{report.coachComment}</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-4 px-4">
      ...
      <div className="rounded-3xl bg-white/10 backdrop-blur border border-orange-500/20 shadow-sm p-4 relative overflow-hidden">
        <div className="flex items-center gap-1.5 mb-3">
          <span className="text-amber-400">⭐</span>
          <span className="text-sm font-bold text-amber-300">本场最值得看见的成长</span>
          <span className="text-amber-400">⭐</span>
        </div>
        <div className="relative px-2">
          <span className="absolute -top-1 -left-0 text-4xl font-black text-orange-200 leading-none select-none">"</span>
      ...
      {/* Video section */}
      {clips.length > 0 && clip && (
        <div className="rounded-3xl bg-white/10 backdrop-blur border border-orange-500/20 shadow-sm overflow-hidden">
          <div className="flex items-center gap-2 px-4 pt-4 pb-2">
            <span className="text-base">▶️</span>
            <span className="text-sm font-bold text-white">本场精彩瞬间</span>
          </div>
            ...
            {clips.map((c, i) => (
              <button
                ...
                className={`shrink-0 text-xs px-3 py-1 rounded-full border font-medium transition-colors ${
                  i === activeClip ? "bg-orange-400 text-white border-orange-400" : "bg-white/10 border-orange-500/30 text-orange-300"
                }`}
              >
                {c.title}
              </button>
          ...
          <div className="flex gap-2 px-3 pb-4">
            ...
            <div className="w-28 shrink-0 flex flex-col justify-center">
              <div className="bg-amber-500/10 border border-amber-200 rounded-2xl rounded-tl-sm p-2.5 relative">
                <div className="text-xs font-semibold text-amber-300 mb-1">教练点评：</div>
                <p className="text-xs text-gray-200 leading-snug line-clamp-4">{clip.coachComment}</p>
              </div>
              <div className="text-xs text-orange-400 mt-1.5 text-center">♡</div>
            </div>
          </div>
          <div className="mx-4 mb-4 bg-orange-500/10 rounded-2xl p-3">
            <div className="text-xs font-medium text-orange-300 mb-1">👨‍👦 写给家长的话</div>
            <p className="text-xs text-gray-200 leading-relaxed">{clip.parentExplanation}</p>
          </div>
        </div>
      )}
      ...
      <div className="rounded-3xl bg-white/10 backdrop-blur border border-orange-500/20 shadow-sm p-4">
        <div className="flex items-center gap-1.5 mb-3">
          <span className="text-base">⭐</span>
          <span className="text-sm font-bold text-white">成长雷达图</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex flex-col gap-3 w-20 shrink-0">
            {radarLeft.map(({ key, emoji }) => {
              ...
              if (!d) return null;
              return (
                <div key={key} className="flex flex-col items-center bg-orange-500/10 rounded-2xl p-2">
                  <span className="text-xl mb-0.5">{emoji}</span>
                  <div className="text-xs text-slate-400 leading-tight text-center">{key}</div>
                  <div className="text-base font-black text-orange-300">{d.score}分</div>
                  <div className="text-xs text-green-400 font-medium">进步↑{scoreDeltas[key]}</div>
                </div>
              );
            })}
          </div>
      ...
      {report.metrics && (
        <div className="rounded-3xl bg-white/10 backdrop-blur border border-orange-500/20 shadow-sm p-4">
          <div className="flex items-center gap-1.5 mb-3">
            <span className="text-base">📊</span>
            <span className="text-sm font-bold text-white">数据统计</span>
          </div>
      ...
      <div className="rounded-3xl bg-white/10 backdrop-blur border border-green-500/20 shadow-sm p-4">
        <div className="flex items-center gap-1.5 mb-3">
          <span className="text-base">✅</span>
          <span className="text-sm font-bold text-white">进步点</span>
        </div>
        <ul className="flex flex-col gap-2">
    ...
    <div className="flex flex-col gap-4 px-4">
      ...
      {report.trendData && (
        <div className="rounded-3xl bg-white/10 backdrop-blur border border-orange-500/20 shadow-sm p-4">
          <div className="flex items-center gap-1.5 mb-3">
            <span className="text-base">📈</span>
            <span className="text-sm font-bold text-white">阶段成长趋势</span>
          </div>
          <GrowthCurve />
        </div>
      )}
      ...
      {/* Coach joint review */}
      <div className="rounded-3xl bg-white/10 backdrop-blur border border-amber-500/20 shadow-sm p-4">
        <div className="flex items-center gap-2 mb-3">
          <span className="text-base">🎓</span>
          <span className="text-sm font-bold text-white">本月教练联合点评</span>
        </div>
        ...
        {report.internalCoachNotes && (
          <div className="mt-3 bg-sky-500/10 rounded-2xl p-3">
            <div className="text-xs font-medium text-sky-300 mb-1">专项建议</div>
            <p className="text-xs text-gray-300 leading-relaxed">{report.internalCoachNotes}</p>
          </div>
        )}
      </div>
      ...
      <div className="rounded-3xl bg-white/10 backdrop-blur border border-green-500/20 shadow-sm p-4">
        <div className="flex items-center gap-1.5 mb-3">
          <span className="text-base">📋</span>
          <span className="text-sm font-bold text-white">下阶段训练计划</span>
        </div>
        <ul className="flex flex-col gap-2">
          {report.suggestions.map((s, i) => (
            <li key={i} className="flex gap-2 text-sm">
              <span className="w-5 h-5 rounded-full bg-green-500/20 text-green-300 flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">{i + 1}</span>
              <span className="text-gray-200">{s}</span>
            </li>
          ))}
        </ul>
...
export default function ReportDetailPage() {
  ...
  return (
    <div className="-mx-4 -mt-6 pb-10 text-white" style={{ background: "radial-gradient(circle at 15% 0%, rgba(255,132,39,0.22), transparent 30%), radial-gradient(circle at 85% 12%, rgba(255,212,71,0.12), transparent 34%), linear-gradient(180deg, #101B2D 0%, #07111F 58%, #05070D 100%)" }}>
      {/* Top hero */}
      <div className="relative flex flex-col items-center pt-8 pb-4 px-4">
        ...
        <div className="flex items-center gap-2 mb-1">
          <h1
            ...
            style={{ color: "#EDF4FF", fontFamily: "PingFang SC, Hiragino Sans GB, sans-serif" }}
          >
        ...
        <div className="flex items-center gap-2 flex-wrap justify-center">
          <div className="flex items-center gap-1.5 bg-white/10 backdrop-blur border border-orange-500/30 rounded-full px-3 py-1">
            <span className="text-orange-500 text-xs">🗓</span>
            <span className="text-xs font-medium text-orange-300">{report.createdAt} {sceneLabel[report.scene] ?? report.scene}</span>
        ...
        <button
          ...
          className={`w-full rounded-3xl py-4 text-base font-bold transition-all active:scale-95 ${
            liked
              ...
              : "bg-white/10 border border-orange-500/30 text-orange-300 hover:bg-orange-500/10"
          }`}
        >
      ...
      <div className="px-4 pt-3 pb-2">
        <button
          ...
          className={`w-full rounded-2xl py-3 text-sm font-bold border transition-colors active:opacity-70 ${shareCopied ? "bg-green-500/15 border-green-300 text-green-400" : "bg-white/10 border-orange-500/30 text-orange-300"}`}
        >
